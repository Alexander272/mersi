...
import { useGetRepairQuery } from '../../repairApiSlice'
...
import { EditRepairItem } from './EditItem'

type Props = {
	instrumentId: string
}

export const EditRepairList: FC<Props> = ({ instrumentId }) => {
	...
	return (
		<>
			...
			<List>
				{data?.data.map(e => {
					const title = `Ремонт от ${new Date(e.periodStart).toLocaleDateString()}`
					return (
						<Fragment key={e.id}>
							<ListItemButton
							...
							<Collapse in={open === e.id} timeout='auto'>
								<EditRepairItem data={e} instrumentId={instrumentId} />
							</Collapse>
