...
import { useGetTransferToDepartmentQuery } from '../../transferApiSlice'
...
import { EditTransferToDepItem } from './EditItem'

type Props = {
	instrumentId: string
}

export const EditTransferToDepList: FC<Props> = ({ instrumentId }) => {
	const [open, setOpen] = useState('')

	const { data, isFetching } = useGetTransferToDepartmentQuery(instrumentId, { skip: !instrumentId })
	...
	return (
		<>
			...
			<List>
				{data?.data.map(e => {
					const title = `Передача от ${new Date(e.date).toLocaleDateString()}`
					return (
						<Fragment key={e.id}>
							<ListItemButton
