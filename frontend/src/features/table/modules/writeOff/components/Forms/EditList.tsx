...
import { useGetWriteOffQuery } from '../../writeOffApiSlice'
...
import { EditWriteOffItem } from './EditItem'

type Props = {
	instrumentId: string
}

export const EditWriteOffList: FC<Props> = ({ instrumentId }) => {
	const [open, setOpen] = useState('')

	const { data, isFetching } = useGetWriteOffQuery(instrumentId, { skip: !instrumentId })
	...
	return (
		<>
			...
			<List>
				{data?.data.map(e => {
					const title = `Списание от ${new Date(e.date).toLocaleDateString()}`
					return (
						<Fragment key={e.id}>
							<ListItemButton
