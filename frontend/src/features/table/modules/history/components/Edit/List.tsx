...
import type { IHistoryForm, IHistoryTypeDTO } from '../../types/historyTypes'
...
export const HistoryList: FC<Props> = ({ section }) => {
	const dispatch = useAppDispatch()

	const { data, isFetching } = useGetHistoryTypesQuery(section, { skip: !section || section == 'new' })
	const [create, { isLoading: creating }] = useCreateHistoryTypesMutation()
	const [updateAll, { isLoading: updating }] = useUpdateHistoryTypesMutation()
	...
	const openDialog = () => {
		dispatch(changeDialogIsOpen({ variant: 'EditHistory', isOpen: true }))
	}
	...
	return (
		<>
			...
			<HistoryDialog submit={submitHandler} />
		</>
	)
}
