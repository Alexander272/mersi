...
import { VerificationHistoryForm } from './Form'
...
export const VerificationHistoryDialog: FC<Props> = ({ submit }) => {
	const modal = useAppSelector(getDialogState('EditVerificationHistory'))
	...
	return (
		<Dialog
			title={context?.data ? 'Редактировать колонку' : 'Добавить колонку'}
			headerActions={<DeleteComponent {...(modal?.context as Context)} submit={submit} />}
			body={<VerificationHistoryForm {...context} submit={submit} />}
...
const DeleteComponent: FC<{ data?: IVerificationFieldForm; submit: (data: IVerificationFieldForm) => void }> = ({
	...
	return (
		<Confirm
			...
			confirmText={`Вы уверены, что хотите удалить колонку "${data?.label}"?`}
		/>
	)
}
